...
export function TestPaymentProviderConnectionStatusForm() {
  const testConnection = api.settings.testPaymentProviderConnection.useMutation(
    {
      onSuccess: (data) => {
        toast.success(data.success ? "Success" : "Failed", {
          description:
          ...
          render={() => (
            <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
              <div className="space-y-0.5">
                <FormLabel className="text-base">Payment Provider</FormLabel>
                <div className="flex items-center gap-2">
                  <FormDescription>
                    Test the connection to your payment provider.
                  </FormDescription>
                  <Dialog>
                    {testConnection.data && (
